...
public class InstructorScheduleReportServlet extends HttpServlet {
    ...
    private static final BaseColor ROW_ALT    = new BaseColor(239, 246, 255);
    private static final BaseColor ACCENT     = new BaseColor(59, 130, 246);
    private static final BaseColor FOOTER_TXT = new BaseColor(107, 114, 128);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String instructorName = userId;
        String instructorId   = "";

        try (Connection conn = getMySQLConnection()) {
            String nameSql = "SELECT INST_ID, CONCAT(FNAME, ' ', LNAME) AS fullname FROM INSTRUCTOR WHERE USER_ID = ?";
            try (PreparedStatement ps = conn.prepareStatement(nameSql)) {
                ps.setString(1, userId);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        instructorId   = rs.getString("INST_ID");
                        String fn      = rs.getString("fullname");
                        if (fn != null && !fn.trim().isEmpty()) instructorName = fn.trim();
                    }
                }
            }
        ...
        String filename    = "INSTRUCTOR_SCHEDULE_" + timestamp + ".pdf";
        ...
        try (OutputStream out = response.getOutputStream()) {
            generateSchedulePDF(out, appName, instructorName, instructorId, userId, generatedAt);
        } catch (DocumentException e) {
            throw new ServletException("PDF generation failed: " + e.getMessage(), e);
        }
    }

    private void generateSchedulePDF(OutputStream out, String appName, String instructorName,
                                     String instructorId, String userId, String generatedAt)
            throws DocumentException, IOException {

        Document doc = new Document(PageSize.A4.rotate(), 36, 36, 80, 60);
        PdfWriter writer = PdfWriter.getInstance(doc, out);
        writer.setPageEvent(new HeaderFooterEvent(appName, instructorName, generatedAt, "Instructor Schedule Report"));
        doc.open();

        // ── Report title block ────────────────────────────────────────────────
        Paragraph title = new Paragraph("Instructor Weekly Teaching Schedule", FONT_TITLE);
        title.setAlignment(Element.ALIGN_CENTER);
        title.setSpacingAfter(4);
        doc.add(title);

        Paragraph sub = new Paragraph("Schedule overview for: " + instructorName
                + (instructorId.isEmpty() ? "" : "  |  ID: " + instructorId), FONT_SUB);
        ...
        String sql = "SELECT s.SCHED_ID, s.DAY_OF_WEEK, s.TIME_START, s.TIME_END, "
        ...
        for (String day : DAY_ORDER) {
            ...
            // Data table: Schedule ID | Course Code | Course Name | Time Slot
            PdfPTable table = buildTableShell(new float[]{20f, 18f, 42f, 24f});
            addHeaderCell(table, "Schedule ID");
        ...
        if (!anyData) {
            Paragraph noData = new Paragraph("No schedules assigned to this instructor.", FONT_SUB);
    ...
    private static class HeaderFooterEvent extends PdfPageEventHelper {
        ...
        HeaderFooterEvent(String appName, String instructorName, String generatedAt, String reportLabel) {
        ...
        public void onEndPage(PdfWriter writer, Document document) {
            ...
            ColumnText.showTextAligned(cb, Element.ALIGN_RIGHT,
                    new Phrase("Instructor: " + instructorName, FNT_HDR_SUB), ml + usableW - 8, hdrY + 24, 0);
            ColumnText.showTextAligned(cb, Element.ALIGN_RIGHT,
